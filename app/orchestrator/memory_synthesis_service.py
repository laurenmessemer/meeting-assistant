"""Memory synthesis service for synthesizing insights from past context."""

from typing import Dict, Any, Optional, List
from app.llm.gemini_client import GeminiClient


def _sanitize_memory_value(value: Optional[str]) -> str:
    """
...
def _sanitize_past_context(past_context: Optional[List[Dict[str, Any]]]) -> List[str]:
    """
    ...
    for mem in past_context:
        if isinstance(mem, dict):
            value = mem.get("value", "")
            sanitized_value = _sanitize_memory_value(value)
...
class MemorySynthesisService:
    """Service for synthesizing memory insights from past context."""
    ...
    async def synthesize(
        ...
        sanitized_memories = _sanitize_past_context(past_context)
