        ...
        for attempt in range(max_retries):
            try:
                ...
                if "429" in error_str or "quota" in error_str.lower() or "rate limit" in error_str.lower():
                    ...
                    raise Exception(f"Error generating response from Gemini: {error_str}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "429" in error_str or "quota" in error_str.lower() or "rate limit" in error_str.lower():
                    ...
                    # Non-rate-limit error, raise immediately
                    raise Exception(f"Error generating structured response from Gemini: {error_str}")
        
        raise Exception(f"Failed to generate structured response after {max_retries} attempts")
